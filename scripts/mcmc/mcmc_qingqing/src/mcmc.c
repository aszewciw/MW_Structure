...
int degrees_of_freedom(POINTING *p, int lower_ind, int upper_ind){
    ...
    for(i = lower_ind; i < upper_ind; i++){
        for(j = 0; j < p[i].N_bins; j++){
            if( p[i].rbin[j].err_jk_DD<tol) continue;
            if( p[i].rbin[j].err_jk_RR<tol) continue;
            if( p[i].rbin[j].DD<tol) continue;
            dof++;
        }
    }
    return dof;
...
void run_mcmc(POINTING *plist, ARGS args, int lower_ind, int upper_ind, int rank, int nprocs, char filename[256]){
    ...
    calculate_sigma(plist, lower_ind, upper_ind);
    chi2 = calculate_chi2(plist, lower_ind, upper_ind);
