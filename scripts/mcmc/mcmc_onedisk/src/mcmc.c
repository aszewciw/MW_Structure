...
void set_weights(STEP params, POINTING *p, int lower_ind, int upper_ind){
    ...
    for(i = lower_ind; i < upper_ind; i++){
        ...
        for(j = 0; j < p[i].N_stars; j++){
            ...
            p[i].weight[j] = ( sech2( p[i].Z[j] / (2.0 * params.z0_thin) )
...
int check_convergence(STD *std){
    double rthin_std_old, zthin_std_old;
    double diff, tol;
    ...
    int upper_ind, int rank, int nprocs, char filename[256])
{
    ...
    disp[2] = offsetof( STEP, chi2 );
    disp[3] = offsetof( STEP, chi2_red );

    /* build derived data type */
    MPI_Type_create_struct( 4, blocklen, disp, type, &MPI_STEP );
        ...
        if(rank == 0){
            ...
            if(i % 1000 == 0){
                ...
                fprintf(stderr, "z0_thin: %lf, r0_thin: %lf\n",
                    current.z0_thin, current.r0_thin);
            }
