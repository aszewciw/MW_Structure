...
double random_gal_Z(double z0, double pdf_norm, double cdf_const, double z_min,
    double z_max)
{
...
double random_gal_R(double r0, double pdf_norm, double cdf_const, double r_min,
    double r_max)
{
    /*  hard to define these variables: they are steps in inverting
        the cdf to draw from the distribution */
    ...
    int max_steps = 100; /* avoid getting caught in infinite loop */
    int i = 0;
...
void generate_stars( STAR *s, PARAMS *p, int disk_type ){
    ...
    if(disk_type==0){
        ...
        z0_cdf_const = p->z0_cdf_const_thin;
        r0_cdf_const = p->r0_cdf_const_thin;
    }
    else if(disk_type==1){
        /* use thick disk */
        N_stars     = p->N_thick;
        ...
        z0_cdf_const = p->z0_cdf_const_thick;
        r0_cdf_const = p->r0_cdf_const_thick;
    }
    else{
    ...
    for( i=0; i<N_stars; i++ ){
        s[i].gal_z = random_gal_Z(z0, z0_pdf_norm, z0_cdf_const, p->z_min, p->z_max);
        s[i].gal_r = random_gal_R(r0, r0_pdf_norm, r0_cdf_const, p->r_min, p->r_max);
